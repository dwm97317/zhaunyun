...
class PaySuccess extends Basics
{
    ...
    public function onPaySuccess($payType, $payData = [])
    {
        ...
        // 订单支付成功行为
        if ($status == true) {
            $this->model['order_sn'] = $this->model['order_no'];
            $this->model['created_time'] = $this->model['create_time'];
            $this->model['total_free'] = $this->model['total_price'];
            $this->model['id'] = $this->model['order_id'];
