...
class Comment extends CommentModel
{
  ...
  /**
    ...
    {
        return $this->belongsTo('Inpack','order_id')->field(['id', 'order_sn']);
    }
    ...
    // 订单评论详情
    public function getOrderCommentDetails($order_id,$user_id){
         // 筛选条件
        $filter = [
            'goods_id' => $order_id,
            'user_id' => $user_id,
        ...
        return $this->with(['user','Inpack','image.file'])
            ->where($filter)
            ->order(['sort' => 'asc', 'create_time' => 'desc'])
            ->paginate(15, false, [
                'query' => request()->request()
     ...
     * @throws \think\exception\DbException
     */
    public function getHotCommentList()
    {
        // 筛选条件
        $filter = [
            'is_delete' => 0,
        ...
        return $this->with(['user','Inpack','image.file'])
            ->where($filter)
            ->order(['sort' => 'asc', 'create_time' => 'desc'])->limit(3)
            ->select();
    }
     ...
     * @throws \think\exception\DbException
     */
    public function getHotOrderCommentList()
    {
        // 筛选条件
        $filter = [
            'is_delete' => 0,
     ...
     * @throws \Exception
     */
    public function addForPack($order,$post)
    {
        // 生成 formData
        ...
        // 生成评价数据
        $data = $this->createCommentDataForPack($order['member_id'], $order, $formData);
    
        if (empty($data)){
            $this->error = '没有输入评价内容';
            return false;
        }
        return $this->transaction(function () use ($data,$formData,$order) {
            // 记录评价内容
     ...
     * @throws BaseException
     */
    private function createCommentDataForPack($user_id, $order, $formData)
    {
        $data = [];
        ...
        !empty($formData['content']) && $data = [
            ...
            'user_id' => $user_id,
            'order_id' => $order['id'],
            'goods_id' => $order['line_id'],
            'order_goods_id' => $order['line_id'],
            'wxapp_id' => self::$wxapp_id,
            'create_time' => time(),
            'update_time' => time()
        ];
