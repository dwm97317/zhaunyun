...
class Coupon extends CouponModel
{
    ...
    public function getList($user = false, $limit = null, $only_receive = false)
    {
        ...
        $this->where('is_delete', '=', 0);
         $this->where('is_open', '=', 0);
        $coupon_type = input('coupon_type');
        if(isset($coupon_type) && $coupon_type>-1){
            switch (input('coupon_type')) {
                case '0':
                    $this->where('coupon_type', '=', 10);
                    break;
                case '1':
                    $this->where('coupon_type', '=', 20);
                    break;
                case '2':
                    $this->where('coupon_type', '=', 30);
                    break;
                default:
                    $this->where('coupon_type', '=', 10);
                    break;
