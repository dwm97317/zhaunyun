...
class Sendpack extends Basics
{
    ...
    private function onSendWxTplMsg()
    {
        ...
        // 获取订阅消息配置
        $template = SettingModel::getItem('tplMsg', $orderInfo['wxapp_id'])['sendpack'];
        ...
        if ($storesetting['client']['mode']==10) {
            ...
            'data' => [
                $template['keywords'][0] => ['value' => $orderInfo['order_sn']],
                $template['keywords'][1] => ['value' => $orderInfo['t_order_sn']],
                $template['keywords'][2] => ['value' => 1],
                $template['keywords'][3] => ['value' => !empty($orderInfo['t_name'])?$orderInfo['t_name']:'默认'],
