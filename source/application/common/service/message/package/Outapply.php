...
class Outapply extends Basics
{
    ...
    private function onSendWxTplMsg()
    {
        ...
        // 获取订阅消息配置
        $template = SettingModel::getItem('tplMsg', $orderInfo['wxapp_id'])['outapply'];
        ...
        if ($storesetting['client']['mode']==10) {
            ...
            'data' => [
                ...
                $template['keywords'][1] => ['value' => $orderInfo['nickName']],
                $template['keywords'][2] => ['value' => $orderInfo['countpack']],
                $template['keywords'][3] => ['value' => $orderInfo['shopname']],
                $template['keywords'][4] => ['value' => $orderInfo['packtime']],
            ]
            ]);
        }
    }
