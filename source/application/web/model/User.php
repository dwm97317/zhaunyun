...
use app\web\model\dealer\Referee as RefereeModel;
use app\web\model\dealer\Setting as DealerSettingModel;
...
class User extends UserModel
{
    ...
    {
        if ($data['old'] !== yoshop_hash($data['oldpassword'])) {
            $this->error = '原密码不正确';
            return false;
        }
        ...
        // 更新管理员信息
        $res = $this->where('user_id',$data['user_id'])->update([
                'password' => yoshop_hash($data['newpassword']),
            ]);
        if (!$res) {
            return false;
        }
        // 更新session
        unset($_SESSION['yoshop_user']);
        return true;
    }
    ...
    public function edit($user_id,$data){
       $allow_up_field = ['nickName','gender','email','mobile','country','contact_name','contact_address','post_code','real_name'];
       $update = [];
       foreach ($allow_up_field as $v){
           if (isset($data[$v]) &&  $data[$v]){
              $update[$v] = $data[$v];
           }
       } 
       return $this->where(['user_id'=>$user_id])->update($update); 
    }
    ...
    public function modifyPwd($data){
        if (yoshop_hash($data['old_password']) != $this->password){
            return false;
        }
        if ($data['password'] != $data['confirm_password']){
             return false;
        }
        return $this->save(['password'=>yoshop_hash($data['password'])]);
    }
